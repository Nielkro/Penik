...
class E2EECrypto {
    ...
    fun encrypt(plaintext: ByteArray, sharedSecret: ByteArray, info: String = "penik-pairwise-message-v1", aad: ByteArray? = null): E2EEncrypted {
        ...
        val cipher = try {
            ...
            Cipher.getInstance("***REDACTED-BY-FILTER-REPO***")
        }
    ...
    /** Decrypts the attachment format: supports both PCK1 chunked files and legacy monolithic ***REDACTED-BY-FILTER-REPO***. */
    ...
    /** Encrypts file payload using PCK1 chunked format with ***REDACTED-BY-FILTER-REPO*** (if useChunked=true and supported), falling back to monolithic format. */
