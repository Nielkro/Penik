...
def e2ee_encrypt(
    ...
    """
    Encrypts plaintext using HKDF-derived ***REDACTED-BY-FILTER-REPO*** key with random salt & nonce.
...
def e2ee_decrypt(
    ...
    """
    Decrypts ***REDACTED-BY-FILTER-REPO*** ciphertext using HKDF-derived key with salt & nonce.
...
def encrypt_file(file_bytes: bytes) -> Tuple[bytes, bytes]:
    """
    Encrypts file with a random 256-bit ***REDACTED-BY-FILTER-REPO*** key.
