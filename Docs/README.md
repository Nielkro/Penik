...
3. [**System Architecture & Cryptography (`Docs/ARCHITECTURE.md`)**](./ARCHITECTURE.md)
   - Схема End-to-End шифрования (E2EE): X3DH, X25519, ***REDACTED-BY-FILTER-REPO***.
   ...
   - Клиентское шифрование, загрузка и стриминг зашифрованных вложений через собственное защищённое хранилище (HTTP Range).

4. [**LiveKit Calls & Signaling (`Docs/CALLS.md`)**](./CALLS.md)
   - Архитектура 1:1 аудио- и видеозвонков на базе LiveKit.
   - Описание сигнализации (опкоды `0x24` – `0x29`).
   - Переменные окружения и отказ при отсутствии конфигурации (Fail-Closed).
