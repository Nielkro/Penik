...
Клиенты (`client/js/call.js`, `android/app/src/main/java/niel/kro/penik/domain/call/CallManager.kt`) сверяют `call_id` каждого входящего кадра с тем звонком, в котором находятся (`_isCurrentCall` / `isCurrentCall`), и обрабатывают `CALL_TAKEN` как локальную остановку звонка без отправки чего-либо на сервер.
