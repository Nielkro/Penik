...
- `rust/penik-crypto/src/cipher.rs` — ChaCha20-Poly1305 encryption, decryption, streaming chunked file encryption/decryption (`PCK1` format), and batch pairwise fan-out message encryption.
...
- `client/js/crypto.js` — Browser cryptography module powered by the `penik-crypto` WebAssembly micro-core with chunked streaming file encryption (`encryptBlobChunked`), `encryptPairwiseBatch`, and WebCrypto vault integration.
...
- `android/app/src/main/java/niel/kro/penik/data/crypto/E2EECrypto.kt` — Implements Android E2EE using X25519, HKDF, and ChaCha20-Poly1305, delegated to native Rust crypto (`RustCryptoCore`) with batch pairwise fan-out (`encryptPairwiseBatch`), streaming chunked file operations (`encryptFileStream`), key backups, and fallback routines.
...
- `tests/e2e/crypto_utils.py` — Python cryptographic helper powered by the Rust `penik-crypto` micro-core via C-ABI / ctypes for X25519, ChaCha20-Poly1305, pairwise AAD v1/v2, PBKDF2, HKDF, memory zeroize, file crypto, and safety fingerprinting.
- `tests/e2e/test_crypto_core.py` — Direct Python verification suite validating Rust C-ABI crypto primitives (PBKDF2, HKDF, AAD v2, zeroize, DH, ChaCha20-Poly1305, file crypto, safety numbers) against Python standard reference implementations.
...
- `tests/e2e/test_runner.py` — Automated E2E test suite running 34 assertions across live auth, key bundles, dual-client live E2EE messaging, offline queuing, attachments, safety numbers, and groups; supports auto-starting ephemeral test servers with isolated databases.
