...
- `rust/penik-crypto/src/cipher.rs` — ***REDACTED-BY-FILTER-REPO*** encryption, decryption, file encryption, and E2EE message transforms.
...
- `android/app/src/main/java/niel/kro/penik/data/crypto/E2EECrypto.kt` — Implements Android E2EE using X25519, HKDF, and ***REDACTED-BY-FILTER-REPO***, delegated to native Rust crypto (`RustCryptoCore`) with fallback to Java security, including key backups and file attachment encryption (`encryptFileChaCha20`).
...
- `tests/e2e/crypto_utils.py` — Python cryptographic helper powered by the Rust `penik-crypto` micro-core via C-ABI / ctypes for X25519, ***REDACTED-BY-FILTER-REPO***, pairwise AAD, file crypto, and safety fingerprinting.
