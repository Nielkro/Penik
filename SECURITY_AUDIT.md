...
| R7 | Требует проверки отзыва | Секреты присутствуют в истории Git | Gitleaks: 4 срабатывания за 765 коммитов; актуальность учётных данных не проверялась |
...
#### R7 — секреты в истории

Gitleaks обнаружил записи VK-токенов в `server/.env:5` (коммит `b27e4329`), `Developments/VkUpload/main.go:187` (`2d565ba3`), `Dependencies/VkUpload/main.go:187` (`4518b9f8`), а также потенциальный TURN credential в `client/js/call.js:192` (`d5e9a6ec`). Это четыре записи, не обязательно четыре разных секрета. Значения намеренно не включены в отчёт. Действительность и факт прежнего отзыва не установлены.

Действие: проверить отзыв обнаруженных учётных данных, заменить действующие; удаление из текущего дерева не удаляет значения из истории. Статические TURN credentials в распространяемом клиенте следует заменить краткоживущими.
...
Приоритет: R1, R3, R4 и проверка действительности исторических секретов, затем ACL группы, валидация X25519 и обновление зависимостей. Первичная оценка R2 как двух независимых высокорисковых отказов сервера скорректирована после практической проверки. Исправления кода в рамках этой проверки не вносились.
