...
## Повторная проверка — 2026-09-11 (Статус: Все уязвимости устранены 2026-09-13)

Проверен код на базе `6341f3d`: серверные обработчики и middleware, WebSocket, Rust-криптография, основные потоки ключей и вложений Web/Android. Находки R1–R6 устранены 2026-09-13 (коммиты `34aceee`, `9a1a991`, `601f140`, `718cf30`), покрыты Go unit-тестами, Rust-тестами и проверены сквозным E2E integration test suite (67 проверок), встроенным в качестве обязательного блокирующего гейткипера перед деплоем в CI/CD.

### Сводная таблица находок повторной проверки

| ID | Исходный риск | Проблема | Статус | Подтверждение и устранение |
| --- | --- | --- | --- | --- |
| R1 | Высокий | Отозванный токен сохраняет доступ через открытый WebSocket | **Исправлено** (`34aceee`, `9a1a991`) | Вызов `hub.CloseSession(tokenHash)` при `/logout`, `/logout/all` и смене пароля; сокеты закрываются с кодом `1008 Policy Violation`. Проверка TTL сессии на каждом фрейме. Покрыто `session_close_test.go` и E2E сьютом. |
| R2 | Ограниченный | Panic при декодировании MsgPack (CVE-2026-32284) | **Исправлено** (`34aceee`, `718cf30`) | Библиотека `github.com/shamaton/msgpack/v2` обновлена с v2.2.0 до v2.4.2; усечённые данные возвращают ошибку вместо panic. В CI настроен регулярный `govulncheck`. |
| R3 | Высокий | Вложения буферизуются в RAM; риск исчерпания памяти | **Исправлено** (`34aceee`) | Убран вызов `ParseMultipartForm(210MB)`. Загрузка переведена на потоковый `multipart.Reader` и `io.LimitReader` с чанками 64 KiB прямо на диск без буферизации в RAM. |
| R4 | Высокий | REST-отправка обходит лимиты размера, числа устройств и частоты WebSocket | **Исправлено** (`34aceee`, `9a1a991`) | В `POST /api/v1/messages/send` внедрены лимиты: <=50 устройств и <=128 KiB ciphertext на устройство. FCM push ограничен строго валидированными доставками, чужие `device_id` отбрасываются. Покрыто E2E тестами. |
| R5 | Средний | Исключённый участник может редактировать свои сообщения группы | **Исправлено** (`34aceee`, `9a1a991`) | `handleGroupMessageEdit` требует `status = 'active'` в `group_members`. Offline edits фильтруются по активным участникам. Покрыто E2E тестами. |
| R6 | Средний | X25519 принимает нулевой общий секрет (неконтрибутивный ключ) | **Исправлено** (`34aceee`) | В `rust/penik-crypto/src/keys.rs` добавлена проверка `shared.as_bytes() == &[0u8; 32]` с ошибкой `CryptoError::InvalidKey`. Покрыто `wire_compat_tests.rs`. |
...
- **Статус:** **Исправлено** (коммиты `34aceee`, `9a1a991`).
- **Реализация:**
...
- **Статус:** **Исправлено** (коммиты `34aceee`, `718cf30`).
- **Реализация:**
...
- **Статус:** **Исправлено** (коммит `34aceee`).
- **Реализация:**
...
- `python3 scripts/run_e2e.py` — полный интеграционный сьют (67 проверок, 13 сценариев) проходит за ~8.3 с:
