...
| R2 | Высокий | Достижимые DoS-уязвимости MsgPack | `govulncheck`: GO-2026-4740 и GO-2026-4513, `github.com/shamaton/msgpack/v2@v2.2.0` |
...
`go run golang.org/x/vuln/cmd/govulncheck@latest ./...` обнаружил два пути к известным DoS-уязвимостям: [GO-2026-4740](https://pkg.go.dev/vuln/GO-2026-4740), [GO-2026-4513](https://pkg.go.dev/vuln/GO-2026-4513). В выводе сканера исправленная версия не указана. Входные данные контролирует авторизованный клиент. Ограничение размера кадра само по себе не доказывает безопасность декодирования.

Исправление: выбрать исправленный релиз после проверки advisory либо заменить/исправить декодер; добавить ограничения вложенности и выделения памяти. Разрушительное воспроизведение DoS не выполнялось.
...
- `npm audit` обнаружил `nanoid@3.3.16` ([GHSA-2v37-7h3g-55p8](https://github.com/advisories/GHSA-2v37-7h3g-55p8), high по npm) и `postcss@8.5.19` ([GHSA-fxqj-rqcc-2cmp](https://github.com/advisories/GHSA-fxqj-rqcc-2cmp), moderate). Обе зависимости приходят через Vite и относятся к сборочному инструментарию. Эксплуатация через работающий мессенджер не подтверждена; обновить lockfile до исправленных версий.
...
- `govulncheck` — 2 достижимые уязвимости; дополнительно 4 на уровне импортируемых пакетов и 17 на уровне модулей без обнаруженного пути вызова, не включены как подтверждённые уязвимости приложения.
- `npm audit` — 2 предупреждения транзитивных зависимостей сборки.
- `gitleaks git --redact` — 765 коммитов, 4 срабатывания.

Приоритет: R1–R4 и проверка действительности исторических секретов, затем ACL группы и валидация X25519. Исправления кода в рамках этой проверки не вносились.
