...
Файлы предварительно шифруются на клиенте (***REDACTED-BY-FILTER-REPO***) и загружаются напрямую на сервер (`POST /api/v1/attachments/upload`). Сервер сохраняет зашифрованные бинарные блобы на диск в `UPLOAD_DIR/attachments/` и отдаёт их по `GET /api/v1/attachments/file/:id` с поддержкой HTTP Range (`206 Partial Content`) для надёжной докачки. Ключи дешифрования передаются только внутри защищённых E2EE сообщений и никогда не попадают на сервер. В вебе Service Worker перехватывает `/sw-stream/<id>`, расшифровывает блоб на лету и отдаёт чанки плеерам для бесшовного воспроизведения и перемотки.
