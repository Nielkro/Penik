...
Мессенджер со сквозным шифрованием: Go-бэкенд, веб-клиент на ванильном JS и Android-клиент на Compose. Мульти-девайс, личные и групповые чаты, бинарный WebSocket-протокол на MessagePack.
...
| Криптография | X25519, HKDF, ***REDACTED-BY-FILTER-REPO***, AES-GCM, PBKDF2 (600k итераций) |
...
`.env` и `*.env` в `.gitignore` — секреты в репозиторий не попадают.

## Архитектура

### Сквозное шифрование

**Личные сообщения.** У каждого устройства своя долговременная пара X25519 (identity key). Общий секрет выводится через X25519, из него по HKDF со случайной 32-байтной солью — ключ сообщения (`info: penik-pairwise-message-v1`), шифрование ***REDACTED-BY-FILTER-REPO*** со случайным nonce. Приватный ключ в вебе лежит в IndexedDB, на Android — в зашифрованном хранилище; на сервер уходит только публичная часть.
...
Проект проходил аудит — результаты в `SECURITY_AUDIT.md` (реестр находок со статусами) и `AUDIT.md` (разбор клиентской криптографии).

Устранено: приватный IK перенесён из localStorage в IndexedDB, Room-БД на Android зашифрована SQLCipher, `allowBackup` выключен, добавлен rate limit на key bundle, унифицированы итерации PBKDF2.
